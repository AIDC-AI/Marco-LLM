...
#   return sentences
_PREFIXES = (
    r"(Mr|St|Mrs|Ms|Dr|Capt|Cpt|Lt|Mt|Hr|Fr|Prof|M|Mme|Mlle|Sr|Sra|Srta|"
    r"Sig|Sig.ra|Dott|Dott.ssa|Dhr|Mevr|p|Sr|Sra|Doutor|Doutora|г-н|г-жа|д-р|пан|пані|"
    r"p|pí|pani|pán|κ|κα|dr|hgy|Dl|Dna|cənab|xanım|Мырза|Ханым|Bpk|Ibu|Sdr|Sdri|Tn|Bwana|Bi|Bw|Mr|Mrs|Dr|Prof|Av|Sn|박사님|교수님)[.]"
)
_WEBSITES = r"[.](com|net|org|io|gov|de|fr|es|it|nl|pl|pt|ru|ua|cz|sk|gr|hu|ro|az|kz|sa|ae|eg|pk|id|in|bd|np|my|th|vn|tz|ke|ng|tr|cn|jp|kr)"
_DIGITS = r"(\d)"
_MULTIPLE_DOTS = r"\.{2,}"
_ALPHABETS = (
    r"([A-Za-zÀ-ÖØ-öø-ÿА-Яа-яЁёƏəĞğŞşÇçҚқҢңҮүҰұӨөӘәء-يآ-یא-תऀ-९অ-ঌএ-ঐও-নপ-রলশ-হৎ-৯ก-ฮ\u4e00-\u9fff\u3040-\u309F\u30A0-\u30FF\u3130-\u318F\uAC00-\uD7A3])"
    # 包括汉字、日语假名和韩文字母范围
)
_ACRONYMS = r"([A-Z][.][A-Z](?:[.][A-Z])*[.])"
_STARTERS = (
    r"(The|A|An|This|That|Those|These|There|Der|Die|Das|Le|La|Les|El|La|Lo|Los|Las|"
    r"Il|I|Gli|Le|De|Het|Een|To|Ta|Te|O|A|Os|As|Toto|Tato|To|On|Oni|Tento|Táto|Toto|"
    r"Αυτό|Αυτή|Αυτά|Екейтіні|Екейтініз|Ez|Ezek|Az|Azt|Ő|Ők|Acest|Aceasta|Acești|"
    r"Aceste|Bu|Bunlar|Қайсыбір|Солар|זה|זו|אלה|אלו|Ini|Itu|Apa|Mana|यह|वह|वे|ये|เอ|น|นั่น|นี้|"
    r"โย|เหล่านี้|นั้น|นี้|นั้น|Huyu|Hawa|这一|这个|那个|이것|저것|그것|However|But|Although|Nevertheless|Nonetheless|Yet|Still|Even\sThough|"
    r"然而|しかし|그러나|Однако|Но|Cependant|Sin\sEmbargo|Jedoch|Tuttavia|No\sEntanto|لكن|ومع\sذلك|على\sالرغم\sمن\sذلك|بل)"
)
_SUFFIXES = (
    r"(Inc|Ltd|Jr|Sr|Co|Corp|AG|GmbH|KG|et al|SARL|SA|SAS|dll|yaitu|s.b|etc|SpA|bv|sp. z o.o.|s.r.o.)"
)


def split_into_sentences(text):
    """Split the text into sentences, supporting multiple languages including Korean."""
    text = " " + text + "  "
    text = text.replace("\n", " ")

    # Initial processing for various suffixes and prefixes in multiple languages
    text = re.sub(_PREFIXES, "\\1<prd>", text)
    text = re.sub(_WEBSITES, "<prd>\\1", text)
    text = re.sub(_DIGITS + "[.]" + _DIGITS, "\\1<prd>\\2", text)
    text = re.sub(
    ...
    # Handling quotes, ellipses, and sentences for various languages
    text = text.replace('“', '“')
    text = text.replace('„', '„')
    text = text.replace('«', '«')
    text = text.replace('»', '»')
    ...
    # Handling sentence boundary markers for multiple languages, including Korean
    text = text.replace(".", ".<stop>")
    text = text.replace(";", ";<stop>")
    text = text.replace("ฯ", "ฯ<stop>")
    text = text.replace("?", "?<stop>")
    text = text.replace("!", "!<stop>")

    # Specific handling for Chinese, Japanese, and Korean sentence terminators
    text = text.replace("。", "。<stop>")  # Chinese and Japanese period
    text = text.replace("？", "？<stop>")  # Chinese and Japanese question mark
    text = text.replace("！", "！<stop>")  # Chinese and Japanese exclamation mark
    text = text.replace("……", "……<stop>")  # Chinese and Japanese/Korean ellipsis
    text = text.replace("〜", "〜<stop>")  # Japanese wave dash
    text = text.replace("…", "…<stop>")  # Korean ellipsis

    # Remove temporary markers
    text = text.replace("<prd>", ".")

    # Split text by custom stop markers
    sentences = text.split("<stop>")
...
def count_words(text):
    """
    Counts words/characters for mixed-language text.
    Returns total count of words/characters.
    """
    # 定义语言特征的Unicode范围
    lang_ranges = {
        # CJK语言：按字符计数
        'zh': r'\p{Han}',  # 中文
        'ja': r'[\p{Hiragana}\p{Katakana}\p{Han}]',  # 日语
        'ko': r'[\p{Hangul}\p{Han}]',  # 韩语
        ...
        'th': r'\p{Thai}',  # 泰语
        'bn': r'\p{Bengali}',  # 孟加拉语
        'ne': r'\p{Devanagari}',  # 尼泊尔语
    }

    def count_by_script(text):
        total_count = 0
        remaining_text = text

        # 首先处理需要字符计数的语言
        for script, pattern in lang_ranges.items():
            # 找出当前语言的所有字符
            chars = regex.findall(pattern, remaining_text)
            if chars:
                # 加上字符数
                total_count += len(chars)
                # 从文本中移除已计数的字符
                remaining_text = regex.sub(pattern, ' ', remaining_text)

        # 处理剩余的拉丁文本（按词计数）
        remaining_text = ' '.join(remaining_text.split())  # 规范化空格
        if remaining_text:
            tokenizer = RegexpTokenizer(r"\w+")
            latin_words = tokenizer.tokenize(remaining_text)
