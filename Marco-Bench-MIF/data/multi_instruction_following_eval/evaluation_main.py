...
_INPUT_DATA = flags.DEFINE_string(
    ...
    "path to input data", required=False
)

_INPUT_RESPONSE_DATA = flags.DEFINE_string(
    "input_response_data", "./data/input_response_data.jsonl",
    "path to input response data", required=False
)
...
def test_instruction_following_strict(
    ...
    print(inp.prompt)
    # print(list(prompt_to_response.keys())[18])
...
def print_report(outputs):
    ...
    #   print(f"{instruction_id} {accuracy}")
    results = []
    results.append(f"{prompt_correct / prompt_total:.3f}")
    results.append(f"{instruction_correct / instruction_total:.3f}")

    for instruction_id in sorted(tier1_total.keys()):
        accuracy = tier1_correct[instruction_id] / tier1_total[instruction_id]
        results.append(f"{accuracy:.3f}")

    # Print values as a single line with tab separation
    print("\t".join(results))
