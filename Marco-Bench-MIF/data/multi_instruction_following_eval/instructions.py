...
_CONSTRAINED_RESPONSE_OPTIONS = (
    "My answer is yes.", "My answer is no.", "My answer is maybe.", "Mi respuesta es sí","Mi respuesta es no", "Mi respuesta es tal vez", "Ma réponse est oui", "Ma réponse est non", "Ma réponse est peut-être", "Meine Antwort ist ja.", "Meine Antwort ist nein.", "Meine Antwort ist vielleicht.", "Moje odpověď je ano.", "Moje odpověď je ne.", "Moje odpověď je možná.","Η απάντησή μου είναι ναι", "Η απάντησή μου είναι όχι", "Η απάντησή μου είναι ίσως","আমার উত্তর হ্যাঁ।", "আমার উত্তর না।", "আমার উত্তর হতে পারে।", "我的答案是肯定的。", "我的答案是否定的。", "我的答案是可能。","我的答案是也许。", "Ìdáhùn mi ni bẹ́ẹ̀ni.","Ìdáhùn mi ni bẹ́ẹ̀kọ́.","Ìdáhùn mi ni ó ṣe é ṣe.","התשובה שלי היא כן","התשובה שלי היא לא","התשובה שלי היא אולי","A válaszom igen.","A válaszom nem.","A válaszom az, hogy talán.","Jawaban saya adalah ya.","Jawaban saya adalah tidak.","Jawaban saya adalah mungkin.","La mia risposta è sì.","La mia risposta è no.","La mia risposta è forse.","私の答えははいです。","私の答えはいいえです。","私の答えは多分です。","Менің жауабым иә.","Менің жауабым жоқ.","Менің жауабым мүмкін.","제 대답은 '예'입니다.","제 대답은 '아니요'입니다.","제 대답은 '어쩌면'입니다.","Jawapan saya ya.","Jawapan saya tidak.","Jawapan saya mungkin.","मेरो जवाफ हो हो।","मेरो जवाफ होईन।","मेरो उत्तर हुनसक्छ।","Mijn antwoord is ja.","Mijn antwoord is nee.","Mijn antwoord is misschien.","Moja odpowiedź brzmi tak.","Moja odpowiedź brzmi nie.","Moja odpowiedź brzmi może.","A minha resposta é sim.","A minha resposta é não.","A minha resposta é talvez.","Răspunsul meu este da.","Răspunsul meu este nu.","Răspunsul meu este poate.","Мой ответ — да.","Мой ответ — нет.","Мой ответ — может быть.","Jibu langu ni ndiyo.","Jibu langu ni hapana.","Jibu langu linaweza kuwa.","คำตอบของฉันคือใช่","คำตอบของฉันคือไม่","คำตอบของฉันคืออาจจะ","Cevabım evet.","Cevabım hayır.","Cevabım belki.","Моя відповідь — так.","Моя відповідь — ні.","Моя відповідь — можливо.","Idahun mi jẹ bẹẹni.","Idahun mi jẹ bẹẹkọ.","Idahun mi boya.","我的答案是\"是的\"。","我的答案是\"不是\"","我的答案是\"不好说\"。","Ìdáhùn mi ni ó ṣe é ṣe.","Câu trả lời của tôi là có.","Câu trả lời của tôi là không.","Câu trả lời của tôi là có thể.","میرا جواب ہاں میں ہے۔","میرا جواب نہیں ہے۔","میرا جواب شاید ہے۔", "إجابتي هي نعم", "إجابتي هي لا", "إجابتي هي ربما")
...
# The section spliter.
_SECTION_SPLITER = ("Section", "SECTION", "섹션", "セクション", "ส่วนที่", "РОЗДІЛОМ","Pозділом","SEÇÃO", "Seção", "РАЗДЕЛ","Pаздел", "SEKCÍ", "BAGIAN", "Bagian", "SZAKASZ", "szakasz", "oddílem", "oddíl", "Dnem", "publikem", "částí", "Abschnitt", "ABSATZ", "ABSCHNITT", "Zielgruppe", "ΕΝΟΤΗΤΑ", "Ενότητα", "Ημέρα", "SECCIÓN", "Sección", "BAGIAN", "Bagian","SEZIONE","sezione","BAHAGIAN","Bahagian","SECTIE","Sectie", "SECȚIUNEA", "secțiunea", "Secțiunea", "SEHEMU", "Sehemu", "PHẦN", "Phần", "IPIN", "Abala", "APAKAN", "Apakan", "ÌPÍN","Apá","ÈTÒ","ÈKÓ","Ìpín")
...
class SectionChecker(Instruction):
  ...
  def check_following(self, value):
    ...
    """
    arabic_numerals = r"\d+"
    roman_numerals = r"I|II|III|IV|V|VI|VII|VIII|IX|X|XI|XII|XIII|XIV|XV|XVI|XVII|XVIII|XIX|XX"

    # Combined pattern for section headings
    section_splitter_patten = r"\b" + re.escape(
      self._section_spliter) + r"\s+(?:" + arabic_numerals + r"|" + roman_numerals + r")\b"
...
class ParagraphFirstWordCheck(Instruction):
  ...
  def check_following(self, value):
    ...
    for p in punctuation:
      cleaned_text = cleaned_text.strip(p)

    # 直接使用startswith比较
    return (num_paragraphs == self._num_paragraphs and
            cleaned_text.lower().startswith(self._first_word.lower()))
...
class CommaChecker(Instruction):
  ...
  def check_following(self, value):
    ...
    # 支持多种语言的逗号
    comma_patterns = [',', '，', '،']  # 英文、中文、阿拉伯语逗号
    return not any(comma in value for comma in comma_patterns)
...
class QuotationChecker(Instruction):
  ...
  def check_following(self, value):
    """Checks if the response is wrapped with various styles of double quotation marks."""
    quotes = [
      '"',  # English, Arabic, Hebrew, Indonesian, Nepali
      '""',  # Chinese
      '„"', '„"',  # German
      '«»',  # French, Spanish
      '»«',  # Swiss German
      # 俄语、保加利亚语、乌克兰语、哈萨克语引号
      '«»',  # Russian, Bulgarian, Ukrainian, Kazakh
      '„"', '„"',  # Russian, Bulgarian, Ukrainian, Kazakh
      # 其他语言引号
      '„"', '„"',  # Polish, Czech, Slovak
      '«»',  # Italian, Portuguese
      ...
      '„"', '„"',  # Korean, Japanese (additional styles)
      '«»',  # Hebrew, Arabic (additional styles)
    ]
    ...
    # Iterate over each pair of quote styles
    for quote in quotes:
      if value[0] == quote[0] and value[-1] == quote[-1]:
        return True
